...
from legged_gym.envs.h1_2.h1_2_config import H12Cfg
...
class H12Robot(BaseTask):
    def __init__(self, cfg: H12Cfg, sim_params, physics_engine, sim_device, headless):
    ...
    def compute_observations(self, reset_env_ids):
        ...
        nan_envs_mask = torch.any(torch.isnan(self.obs_buf), dim=1)
        nan_env_ids = nan_envs_mask.nonzero(as_tuple=False).flatten()
        if len(nan_env_ids) > 0:
            print(f"WARNING: NaN detected in observations for envs: {nan_env_ids}. Resetting these environments.")
            # Flag these environments for reset
            self.reset_buf[nan_env_ids] = 1
            
            # Replace NaN values with the last valid observation to prevent them from corrupting the history buffer.
            if self.cfg.env.stack_history_obs and hasattr(self, 'obs_buf_history'):
                # The last valid observation is at index -2 (the one before the current, just-inserted one)
                # We only replace the non-privileged part of the observation.
                last_valid_partial_obs = self.obs_buf_history.obs_buf[:, -2, :]
                self.obs_buf[nan_env_ids, :self.num_partial_obs] = last_valid_partial_obs[nan_env_ids]
            else: # Fallback if history is not available: replace with zeros
                self.obs_buf[nan_env_ids] = 0.0
    ...
    def _get_noise_scale_vec(self, cfg):
        """ Sets a vector used to scale the noise added to the observations.
            ...
            [NOTE]: This function is specific to the H1 V2 observation structure.
        ...
        if not self.add_noise:
            return torch.zeros(self.cfg.env.num_observations, device=self.device)
    ...
    def _generate_symmetry_matrices(self):
        ...
        # ----- Actions Permutation (27 actions) -----
        action_perm = list(range(self.num_actions))
        
        # Left <-> Right Leg
        leg_map = {0: 6, 1: 7, 2: 8, 3: 9, 4: 10, 5: 11}
        for i, j in leg_map.items():
            action_perm[i], action_perm[j] = j, i
        # Negate roll and yaw joints for legs
        for i in [0, 2, 5, 6, 8, 11]: # hip_yaw, hip_roll, ankle_roll
            action_perm[i] = -action_perm[i] if action_perm[i] != 0 else -0.0001

        # Left <-> Right Arm
        arm_map = {13: 20, 14: 21, 15: 22, 16: 23, 17: 24, 18: 25, 19: 26}
        for i, j in arm_map.items():
            action_perm[i], action_perm[j] = j, i
        # Negate roll and yaw joints for arms
        for i in [14, 15, 17, 19, 21, 22, 24, 26]: # shoulder_roll/yaw, elbow_roll, wrist_yaw
            action_perm[i] = -action_perm[i] if action_perm[i] != 0 else -0.0001

        # Torso
        action_perm[12] = -action_perm[12] if action_perm[12] != 0 else -0.0001
